...
void markOuterFunctionCall(Tree curr) {
	if(curr == NULL)
		return;
	if(OP_LABEL(curr) == TYPE_CALL) {
		curr->outer = 1;
		return;
	}
	markOuterFunctionCall(RIGHT_CHILD(curr));
	markOuterFunctionCall(LEFT_CHILD(curr));
}
...
@attributes {struct list * names;} Program Pars  mayPars   
@attributes {char *name;} ID
@attributes {struct list *names; Tree node;} Lexpr Expr mayplus plusExpr maymul mulExpr minusExpr Term Stat Stats Labeldefs Labeldef Cond Cterm andCond beistrichExpr
	...
	@{
		...
		@post {
			if(burm_label(@Funcdef.node@) == 0) {
				...
				markOuterFunctionCall(@Funcdef.node@);
...
beistrichExpr:Expr
	@{
		@e beistrichExpr.names :Expr.names;
		@beistrichExpr.names@ = @Expr.names@;
		
		@i @beistrichExpr.node@ = gen_node(TYPE_FUNPARE, @Expr.node@, NULL, 1, NULL);
	@}
	| Expr COMMA
	@{
		@e beistrichExpr.names :Expr.names;
	...
	| Expr COMMA beistrichExpr
	@{
		@e beistrichExpr.names : beistrichExpr.1.names Expr.names;
		@beistrichExpr.names@ = concatList(@beistrichExpr.1.names@, @Expr.names@);
	    
		@i @beistrichExpr.node@ = gen_node(TYPE_FUNPAR, @beistrichExpr.1.node@, @Expr.node@, @beistrichExpr.1.node@->const_num+1, NULL);
	@}
	;
...
Term: BOPEN Expr BCLOSE 
	...
	@{
		@e Term.names: beistrichExpr.names;
		@Term.names@ = @beistrichExpr.names@;

		@i @Term.node@ = gen_node(TYPE_CALL, @beistrichExpr.node@, NULL, @beistrichExpr.node@->const_num, @ID.name@);
	@}
	;
%%
